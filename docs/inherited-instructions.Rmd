...
```{r results, fig.width=6, fig.height=3}
scores_plot
```

```{r results-final, fig.width=3, fig.height=3}
final_scores_plot
```
...
```{r instructions-coded}
ggplot(InstructionsCoded) +
  aes(dimension) +
...
```{r instructions-outcomes, fig.width=3, fig.height=3}
...
GemsCoded %>%
  filter(instructions_score == 0, score > 80) %>%
  .$subj_id %>%
  unique()

filter(GemsCoded, block_ix <= 2) %>%
  ggplot() +
...
```{r instructions-inherited, fig.width=6, fig.height=3}
generation_map <- select(Gems, subj_id, generation, inherit_from) %>% unique()
instructions_with_generations <- left_join(generation_map, InstructionsCodedSummarized)

gen1 <- instructions_with_generations %>%
  filter(generation == 1) %>%
  select(-inherit_from, -generation) %>%
  rename(inherit_from = subj_id)

gen2 <- instructions_with_generations %>%
  filter(generation == 2) %>%
  select(-instructions_score)

map <- left_join(gen2, gen1)
...
Gems %>%
  ...
  ggplot() +
  aes(trial, score, color = factor(instructions_score)) +
...
_How does the performance of Generation 1 participants in block 2
compare to the performance of Generation 2 participants in block 1?_

Generation 2 will benefit from the instructions left by Generation 1 in block 1
more than Generation 1 in block 2.

```{r post-instructions}
PostInstructions <- Gems %>%
  filter((generation == 1 & block_ix == 2) | (generation == 2 & block_ix == 1)) %>%
  recode_trial()

post_instructions_mod <- lmer(score ~ (trial_z + trial_z_sqr) * generation_c + (trial_z + trial_z_sqr|subj_id),
                              data = PostInstructions)

post_instructions_preds <- expand.grid(trial = 0:80, generation_c = c(-0.5, 0.5)) %>%
  recode_trial() %>%
