...
GemsV1.2Final <- GemsV1.2 %>%
  group_by(generation, subj_id, block_ix) %>%
  filter(trial == max(trial)) %>%
   ungroup()

recode_version <- function(frame) {
  map <- data_frame(version = c(1.1, 1.2), version_f = factor(c(1.1, 1.2)))
  if(missing(frame)) return(map)
  left_join(frame, map)
}

V1_Gen1 <- bind_rows(
  filter(GemsV1.1, generation == 1),
  filter(GemsV1.2, generation == 1)
) %>%
  recode_version()

V1_Gen1_Final <- bind_rows(
  filter(GemsV1.1Final, generation == 1),
  filter(GemsV1.2Final, generation == 1)
) %>%
  recode_version()

# * v1-positions ----
v1_positions_plot <- ggplot(V1_Gen1) +
  aes(x = current_x, y = current_y, xend = selected_x, yend = selected_y) +
  geom_segment(aes(group = subj_id, color = version_f)) +
  facet_wrap("block_ix", nrow = 1) +
  scale_x_continuous(breaks = seq(0, 70, by = 10)) +
  scale_y_continuous(breaks = seq(0, 70, by = 10)) +
  annotate("point", x = 50, y = 50, shape = 4, size = 3) +
...
# * v1-scores ----
v1_scores_plot <- ggplot(V1_Gen1) +
  aes(trial, score) +
  geom_line(aes(group = interaction(subj_id, block_ix), color = version_f), size = 0.2) +
  geom_line(aes(group = version_f, color = version_f), stat = "summary", fun.y = "mean", size = 2,
            show.legend = FALSE) +
  facet_wrap("block_ix", nrow = 1) +
  t_$theme +
  theme(legend.position = "top",
...
# * v1-distance ----
v1_distance_plot <- ggplot(V1_Gen1) +
  aes(trial, distance_2d) +
  geom_line(aes(group = subj_id, color = version_f), size = 0.25) +
  geom_line(aes(group = version_f, color = version_f),
            stat = "summary", fun.y = "mean",
            size = 2, show.legend = FALSE) +
  geom_hline(yintercept = 0, linetype = 2) +
  facet_wrap("block_ix", nrow = 1) +
...
# * v1-final ----
v1_final_scores_plot <- ggplot(V1_Gen1_Final) +
  aes(block_ix, score) +
  geom_line(aes(group = subj_id, color = version_f), size = 0.2) +
  geom_line(aes(group = version_f, color = version_f), stat = "summary", fun.y = "mean", size = 2) +
  geom_hline(yintercept = 100, linetype = 2) +
  xlab("block") +
  t_$theme +
...
v1_final_distances_plot <- ggplot(V1_Gen1_Final) +
  aes(block_ix, distance_2d) +
  geom_line(aes(group = subj_id, color = version_f), size = 0.2) +
  geom_line(aes(group = version_f, color = version_f), stat = "summary", fun.y = "mean", size = 2) +
  xlab("block") +
