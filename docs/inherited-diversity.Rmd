...
around inheritance resulted in more effective problem solving than would be
...
hand off partial solutions to the next generation.

Although the human capacity to work together in teams to solve large problems
is evident in many scientific and technological domains, teamwork is not usually 
...
could have been accomplished individually.

One way in which teamwork has been shown to improve performance over what could
have been accomplished individually is through confidence matching.
@Bahrami:2010jl found that teams of two individuals or dyads performed better
...
best individual in each dyad completing the task alone. Follow-up studies have
shown that dyads were able to improve team performance through confidence
matching. Confidence matching can occur through settling on a common vocabulary
to use when discussing uncertainty, but confidence matching also occurs when
using non-linguistic indicators of uncertainty.
...
Other research has found a benefit to diversity among team members. Diversity
...
agent were passed on to serve as the starting point for attempts by a new agent.
These second generation agents had either the same or different problem solving
skills. We explored whether skill diversity among generatiosn of agents resulted
in better problem solving performance than equally skilled individuals given the
same total number of attempts.
...
unequal skill achieved lower performance scores than agents with equal skills,
...
The results of Experiment 1 depict the phenomena of complementary inheritance,
...
which new solutions are tested by participants seeking to answer the same
problems. In Experiment 2, we test these predictions in a behavioral in
experiment.
...
trained on independent skills, could coordinate their labor so as to improve
...
sampled within a certain distance from the previously selected gem. The
sampling procedure is described in the procedure, below. As participants
selected gems across trials, they effectively traversed the solution landscape
...
`r_$strategies_relative_scores_helmert_mod_resid` (Fig. \@ref(fig:strategies)B).

In terms of distance, a similar pattern was found. Complementary problem
solvers got closer to the best solution than isolated individuals and congruent
strategies.
...
fundamental to human problem solving ability. Yet usually the inheritance of
problem solving knowledge is conceived as being an imperfect process, unable to
result in problem solving performance beyond what could be achieved by
individuals working alone. Here we demonstrate that the inheritance of problem
solving knowledge can in some cases result in team performance greater than
what either individual would have achieved on their own.
...
problem solvers might start future generations off in a part of the problem
space that they were unlikely to achieve on their own. In some cases, this
...
which complementary inheritance might prove effective. We find support for our
primary hypothesis that there are some problems for which teams of
...
and how human problem solvers are so good at inheriting and adapting the tools
and other solutions to problems that were originally discovered by someone
else. Although for many researchers, the advantages of being able to inherit
problem solving knowledge from someone else are not that inheritance affects
your own problem solving ability, only that it gives you a head start on
problem solving, these assumptions need to be tested in rigorous lab
experiments.
