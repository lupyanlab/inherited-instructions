...
abstract: |
  ...
  accumulation of problem solving knowledge over generations such that human
  problem solvers rarely have to start from scratch when solving problems, and
  instead can start off with solutions discovered by a previous generation. Here
  we investigated the impact of inheritance on future problem solving
  performance. We sought to understand the conditions under which inheritance
  could improve problem solving performance. In Experiment 1, we simulated the
  effect of inheritance on problem solving, and discovered that generations of
  complementary problem solvers, who learned different but complementary skills,
  outperformed equally skilled individuals given the same total number of
  trials. In Experiment 2, we tested these predictions in a behavioral
...
[@Tomasello:1993vh; @Tennie:2009gaa; @Dean:2012gv]. As a result, problem
solvers rarely have to start from scratch when solving new problems, and
instead can start off with the solutions that were inherited from a previous
generation. What are the consequences of inheriting from a previous generation
on future problem solving behavior?
...
In Experiment 1, we use agent-based models to simulate the effect of inheritance
on future problem solving performance. In the models, problem solving is defined
as a process of searching a space of possible solutions for the best solution.
The primary constraint on problem solving success is time.
