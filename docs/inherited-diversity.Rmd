...
abstract: |
  ...
  accumulation of problem solving knowledge over generations. As a result of
  our cultural inheritance, we rarely have to start from scratch when solving
  new problems, and instead can benefit from solutions discovered by a previous
  generation. We investigated the impact of inheritance on problem solving
  performance. Specifically, we explored the conditions under which inheritance
  might improve problem solving relative to solving the same problems
  individually. In Experiment 1, we simulated the effect of inheritance on
  problem solving, and demonstrate that there are some problems for which
  generations of complementary problem solvers, with who learned different but
  complementary skills should be able to outperform equally skilled
  individuals. In Experiment 2, we tested these predictions in a behavioral
  experiment. These results illustrate one way in which our system of cultural
  inheritance may result in cumulative improvements to problem solving
...
improve problem solving beyond what could be achieved by an individual.
...
transmission in explaining the nature of human cultural evolution. Sperber and
others [-@Sperber:1996exc; @Claidiere:2010kd] have argued that accurate copying
is possible but rare, and that cultural transmission is better understood as a
transformative process as opposed to a replicative one.
...
Accurate transmission of problem solving knowledge is important for two reasons.
First, it allows others to benefit from complex solutions they themselves could
not have created. Second, it increases the likelihood that future generations
cumulatively improve upon the solutions discovered by their ancestors. High
fidelity transmission is important to human cultural evolution because it means
improvements to tools and other technology can accumulate even if they are
discovered by chance. On this view, the propensity to incrementally improve
solutions to problems inherited from others is an important force in human
cultural evolution.

The primary benefit to being able to inherit problem solving knowledge from
a previous generation is believed to be that it gives future generations a head
start on problem solving. Rather than each new generation striving to solve the
same problems over and over again, future generations can save the time it
would have taken to solve the same problem on their own. Instead, they can spend
their time solving new problems, not attempted by previous generations. As
a result, cultural inheritance allows for problem solving to extend far longer
than any individual lifetime.

Over time, we have come to depend on this system
of cultural inheritance to the point that human cultures are able to solve
problems that no individual could attempt on their own [@Henrich:2015srt;
@Boyd:2017diff].

Importantly, on this view, cultural inheritance improves problem solving
performance without affecting underlying problem solving ability. The inheritance
of problem solving knowledge via social learning is identical to the same
knowledge discovered via individual or asocial learning.

The purpose of this research was to understand the conditions
...
iteratively searching a high dimensional landscape of possible solutions,
...
In Experiment 1, I used agent-based models to simulate the effect of
inheritance on problem solving. The models assume that problem solving is
a process of searching a space of possible solutions for better and better
solutions. To simulate inheritance, the best solution discovered by the
previous generation is used as a starting point for the next generation.

Different agents differ in abilities along two dimensions. Each agent
is assigned the same total amount of ability that is dr

In the models, the only difference between first and second generation
agents is in where they start their search in the solution landscape.

I used the models to understand the conditions under which inheritance
from a previous generation might improve overall problem solving ability.
My hypothesis was that 


The key question was the effect of skill diversity on the effectiveness
of inheritance. When inheritance is minimally defined as a starting
point in a solution landscape, individuals who inherit from others
with identical skills should not be able to do any better or worse
than their ancestors. But inheriting from others who have complementary
skills might allow the team to arrive at solutions that individual
agents might not have discovered on their own.
...
Agents attempted to find the peak of a solution landscape via a random walk
process. In the simulations, for each unit of time, the agents sampled a random
location in the space of possible solutions in a certain distance of the
current location.
...
```{r ability-as-vision, fig.width=6, fig.height=8, fig.cap='(ref:ability-as-vision)'}
grid.arrange(
