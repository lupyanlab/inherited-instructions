...
gridExtra::grid.arrange(
  ...
  training_scores_plot + ggtitle("B. Score"),
  training_distance_plot + ggtitle("C. Distance"),
  nrow = 1
)
```

```{r gems-training-per-trial}
training_score_rank_plot
```

## First generation {-}

(ref:gen1-results) Results of first generation problem solvers. Each line is a
participant. **A. Positions** on the solution landscape. **B. Scores** over
trials.

```{r gen1-results, fig.width=8, fig.height=8, fig.cap="(ref:gen1-results)"}
gridExtra::grid.arrange(
  gen1_positions_plot + ggtitle("A. Positions"),
  gen1_scores_plot + ggtitle("B. Scores"),
  nrow = 2
)
```

(ref:gen1-final-scores) Performance of first generation participants by block.

```{r gen1-final-scores, fig.cap="(ref:gen1-final-scores)"}
