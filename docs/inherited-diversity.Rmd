...
abstract: |
  ...
  problem solving using agent-based models, and demonstrate that there are some
  problems for which generations of complementary problem solvers, who
  learn independent and complementary skills, are able to outperform
  equally skilled individuals. In Experiment 2, we tested these predictions in
  a behavioral experiment. These results illustrate one way in which our system
  of cultural inheritance may result in cumulative improvements to problem
...
# Experiment 1: Agent-based models {-}

```{r peaks-setup}
```

In Experiment 1, we used agent-based models to simulate the effect of
inheritance on problem solving performance. We intended to demonstrate how
skill diversity among generations of problem solvers results in problem
solving performance that no individual could achieve on their own.
To test this, we created problem solving agents that varied in skill
along two independent dimensions. 


inheritance can affect future generations of problem solving by starting the
next generation off in a part of the solution landscape that would be unlikely
to be reached independently. We predicted that as generations of problem
solvers increased in skill diversity, the impact of inheritance as a problem
solving strategy would increase relative to solving the same set of problems as
individuals.
...
The models assume that problem solving can be simulated as a process of
searching a space of possible solutions for better and better answers. To
simulate inheritance over generations, the best solution discovered by the
previous generation---a particular point in the space of possible
solutions---is used as a starting point for the next generation.

The problem solving agents differed along two dimensions of skill. Each skill
corresponds to vision in a particular dimension of the solution landscape. All
agents were allotted the same total amount of skill, which was distributed
between the two dimensions. Some agents had equal skill in both dimensions,
and others had unequal skill in either dimension. We predicted the benefits
of inheritance would be greatest when agents with diverse skills inherit
from one another.

Problem solving agents attempted to find the peak of a solution landscape via a
random walk process. In the simulations, for each unit of time, the agents
sampled a random location a fixed distance from the agent's current location.
This distance is referred to as the agent's search area (Fig.
\@ref(fig:ability-as-vision)B).
