...
inheritance on problem solving performance. Agents were assigned skills corresponding
to vision in two dimensions. Some agents had equal vision in both dimensions, while
others had unequal vision, specializing along one dimension at the expense of
vision in the other. Agents with each type of skill distribution were arranged
in pairs with other agents, forming two agent teams that varied in team
...
skill. We explored whether skill diversity resulted in better problem solving
...
In the simulations, agents completed a problem solving task by searching a space
of possible solutions, looking for the best ones. Agents navigated the solution
landscape using a simple hill climbing procedure. For each time step, agents
randomly sampled a position on the landscape within their range of vision
(described below). If the sampled position was higher on the landscape than the
previous position, then the agent moved to the new position. For the next time
step, a solution was sampled relative to the new position.
...
**Training trial procedure.** Participants were randomly assigned to one of two training conditions. In the
...
**Test trial procedure.** After completing the training trials, participants completed four blocks of 40
test trials. All test trials were conducted on the same simple hill landscape, defined above.
Participants traversed the simple hill landscape four times, once for each block of
40 trials. First generation participants started each block with 6 gems sampled
from around the origin $(0_{ori},0_{bw})$. Second generation participants
participants started off at the location of a first generation participant
after 20 trials, $(x_{ori},y_{bw})$. First and second generation participants
were yoked over blocks such that second generation participants inherited from
the same first generation participant four times total, one for each
block. Participants were given a break after each block, and told that for
