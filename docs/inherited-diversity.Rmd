...
around inheritance may result in more effective problem solving than would be
achieved by solving the same problems individually. To test this, we compared
the performance of individuals working alone to teams of individuals who
hand off partial solutions to be continued by the next generation.
...
Teamwork often introduces social factors such as free-riding
[@Holmstrom:1982cp], social loafing [@Latane:1979in], and bias
[@Lorenz:2011jn] that decrease motivation and performance in the group. Other
...
One situation in which teamwork has been shown to improve performance
over what could have been accomplished individually is in confidence matching.
...
For example, a common argument in economics is that division of labor raises
collective productivity [@Becker:1992fd; @VonHippel:1994jc]. In problem solving
contexts, diversity among team members has been argued to be beneficial in
tasks that require searching a space of possible solutions, looking for the
best ones. For these types of problems, teams of equally skilled problem
solvers may get stuck on suboptimal solutions, where diverse teams are able to
maintain a wider search and in some cases can discover globally optimal
solutions.

As evidence of this, @Hong:2004gt modeled groups of problem solvers, and showed
that groups of diverse problem solvers, with less ability overall, could outperform
higher ability groups. The reason was that high ability groups were necessarily
more similar in problem solving strategy, and thus were more likely to become
stuck on suboptimal solutions to problems. The more diverse group was able
to maintain a wider search, and was less likely to get stuck.

However, a benefit to diversity among group members has not been shown
experimentally. We hypothesized that diversity among group members would
become advantageous in tasks involving the transmission of problem solving
knowledge from one generation to the next. In these cases, the first
generation of problem solvers in a group can search for the best solution
before passing it off to the next generation. Because each generation
has different skills, the first generation can put the second generation
in a part of the solution landscape they would be unlikely to reach on their
own. As a result, the performance of the team may result in better performance
than either individual working alone.
...
work. We find that diverse teams of agents outperform teams of equally
skilled individuals, demonstrating complementary inheritance.

In Experiment 2, we test these predictions in a behavioral experiment.
Participants completed a perceptual decision-making task in which
they navigated a solution landscape looking for the highest quality
space gems. After a certain number of guesses, they pass of their best
guess to the next generation. The next generation, trained on the same
or different skills, continues the work. We predicted that teams
trained on complementary skills would outperform equally skilled individuals.
...
In Experiment 1, we used an agent-based model to illustrate the effect of
...
resulted in better problem solving performance than equally skilled individuals
...
Agents were assigned skill values in two dimensions, corresponding to vision in
a particular dimension of the solution landscape. All agents were allotted the
...
either dimension (Fig. \@ref(fig:ability-as-vision)A).

Agents with equal skill in both dimensions had a square range of vision, and
could sample solutions equally in any direction. Agents with unequal skills had
...
vision relative to their previous best solution. If the sampled solution was
...
was not an improvement, agents sampled from the previous best solution again
...
a crossover effect, `r exp1$crossover`.
...
The results of Experiment 1 demonstrate the phenomena of complementary inheritance,
by which inheritance over generations results in teams of problem solvers
outperforming individuals trained on the same skills and given the same number
of attempts. In the simulations, this improvement in performance is the result
of the first generation putting later generations in a part of the problem space
that would be unlikely to be achieved individually.

However, the value of the agent-based model is limited, especially since the
problem solving process is simulated as a random walk process, where solutions
are selected at random. A more realistic test of whether inheritance can
improve team performance requires a task in which new solutions are selected by
participants seeking to answer the same problems. In Experiment 2, we test
these predictions in a behavioral experiment.
...
trained on independent skills, would coordinate their labor so as to improve
...
(ref:gems-team-structures) Team structures in Experiment 2. Participants were trained to pay
...
they effectively searched the landscape for the highest peaks corresponding
...
an increase in overall score, and negative numbers indicated a decrease.
...
data for 20 first generation participants, and 40 second generation
...
condition. Unlike in the analysis of grid positions reported above, our
...
We focus on two separate but related scoring metrics: 2D distance, corresponding
to the Euclidean distance between each participant and the peak of the test landscape
at $(50,50)$, and score, corresponding to the height or value of the gem
last selected. Each scoring metric can be taken in absolute units,
or in relative units. Relative distance and relative score correspond
to the difference between the starting point and the current point
in terms of distance and score, such that positive numbers mean
the participant was able to move closer to the target and also to improve
the overall score.
...
We found support for our hypothesis that problem solving with inheritance can
...
fundamental to human problem solving ability [@Boyd:2011gp]. Yet the inheritance of
problem solving knowledge can be costly, and it may take years to develop
expertise [@Stout:2002iu]. Here we investigated whether inheritance
can in some cases result in team performance greater than what could be achieved
by individuals working alone. We hypothesized that complementary inheritance,
whereby individuals with independent but complementary skills collaborate
over generations, may result in an overall advantage to inheritance in a
problem solving context.

In Experiment 1 we used an agent-based model to explore the conditions under
...
and how human problem solvers are exceptionally skilled at inheriting and adapting the tools
and other solutions to problems that were originally discovered by someone
else. Here we demonstrate that inheritance over generations may result
in an improvement to problem solving performance, beyond what could be
