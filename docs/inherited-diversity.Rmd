...
abstract: |
  ...
  accumulation of problem solving knowledge over generations. As a consequence,
  problem solvers rarely have to start from scratch when solving new problems.
  Instead, they start off with solutions that were discovered by a previous
  generation. Here we investigated the impact of inheritance---starting off
  a problem solving task with a solution discovered by someone else---on future
  problem solving performance. We sought to understand the conditions under
  which inheritance might improve problem solving performance. We found that
  generations of complementary problem solvers, who learned different but
  complementary skills, outperformed equally skilled individuals given the same
  total number of trials. These results illustrate one way in which our system
  of cultural inheritance results in cumulative improvements in problem solving
...
[@Tomasello:1993rat; @Tennie:2009gaa; @Dean:2012gv]. As a result, problem
...
Tomasello and others [-@Tomasello:1993rat; @Tennie:2009gaa] have argued that
